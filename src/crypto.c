...
static const Field FIELD_MODULUS = {
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x46, 0x98, 0xfc, 0x09, 0x4c,
    0xf9, 0x1b, 0x99, 0x2d, 0x30, 0xed, 0x00, 0x00, 0x00, 0x01};

// Scalar field Fq
static const Scalar GROUP_ORDER = {
    0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x46, 0x98, 0xfc, 0x09, 0x94,
    0xa8, 0xdd, 0x8c, 0x46, 0xeb, 0x21, 0x00, 0x00, 0x00, 0x01};
...
static const Field FIELD_ONE = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01};

static const Field FIELD_TWO = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static const Affine AFFINE_ONE = {
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01},
    {0x1b, 0x74, 0xb5, 0xa3, 0x0a, 0x12, 0x93, 0x7c, 0x53, 0xdf, 0xa9,
     0xf0, 0x63, 0x78, 0xee, 0x54, 0x8f, 0x65, 0x5b, 0xd4, 0x33, 0x3d,
     0x47, 0x71, 0x19, 0xcf, 0x7a, 0x23, 0xca, 0xed, 0x2a, 0xbb}};

void field_copy(Field b, const Field a) { memmove(b, a, FIELD_BYTES); }
...
void field_pow(Field c, const Field a, const Field e) {
  cx_err_t cx_err;
  cx_err =
      cx_math_powm_no_throw(c, a, e, FIELD_BYTES, FIELD_MODULUS, FIELD_BYTES);
...
bool field_is_odd(const Field y) { return y[FIELD_BYTES - 1] & 0x01; }
...
void scalar_copy(Scalar b, const Scalar a) { memmove(b, a, SCALAR_BYTES); }
...
void scalar_pow(Scalar c, const Scalar a, const Scalar e) {
  cx_err_t cx_err;
  cx_err =
      cx_math_powm_no_throw(c, a, e, SCALAR_BYTES, GROUP_ORDER, SCALAR_BYTES);
...
bool scalar_is_zero(const Scalar a) { return scalar_eq(a, SCALAR_ZERO); }

void group_copy(Group *b, const Group *a) { memmove(b, a, sizeof(Group)); }

// zero is the only point with Z = 0 in jacobian coordinates
bool group_is_zero(const Group *p) { return field_eq(p->Z, FIELD_ZERO); }
...
void generate_keypair(Keypair *keypair, const uint32_t account) {
  const uint32_t bip32_path[BIP32_PATH_LEN] = {
      ...
      account | BIP32_HARDENED_OFFSET, 0, 0};

  // Generate private key
  os_perso_derive_node_bip32(CX_CURVE_256K1, bip32_path, BIP32_PATH_LEN,
                             keypair->priv, NULL);
...
bool generate_address(char *address, const size_t len, const Affine *pub_key) {
  ...
  // Encode as address
  int result = b58_encode((unsigned char *)&raw, sizeof(raw),
                          (unsigned char *)address, len);
...
bool message_derive(Scalar out, const Keypair *kp, const ROInput *input,
                    const uint8_t network_id) {
  uint8_t derive_msg[268] = {};
  int derive_len = roinput_derive_message(derive_msg, sizeof(derive_msg), kp,
                                          input, network_id);
  if (derive_len < 0) {
    return false;
  }
  ...
  if (CX_OK !=
      cx_hash_no_throw(&ctx.header, 0, derive_msg, derive_len, NULL, 0)) {
    return false;
  }
  if (CX_OK !=
...
bool message_hash(Scalar out, const Affine *pub, const Field rx,
                  const ROInput *input, const uint8_t network_id) {
  Field hash_msg[9];
  int hash_msg_len =
      roinput_hash_message(hash_msg, sizeof(hash_msg), pub, rx, input);
  if (hash_msg_len < 0) {
    return false;
  }
...
bool sign(Signature *sig, const Keypair *kp, const ROInput *input,
          const uint8_t network_id) {
  Scalar k;
  Affine r;
  Scalar tmp;
